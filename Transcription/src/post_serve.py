...
from src.transcription_utils import update_transcription
...
def merge(base_path, client_id, len):
    ...
    output_filename = f"{client_id}.{len}.wav"
...
def create_app():
    ...
    def submit():
        ...
        print(len)
        # filename = f"{room_id}_modified.txt"
        # write_txt_file(os.path.join("../modified_text_files", filename), edited_text)
        # filename_orig = f"{room_id}_original.txt"
        # write_txt_file(os.path.join("../original_text_files", filename_orig), original_text)

        update_transcription(room_id, "modified", edited_text)
        update_transcription(room_id, "original", original_text)

        merge("../audio_uploads", room_id, len)
        response = {
