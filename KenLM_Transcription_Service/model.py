import kenlm  
from request import ModelRequest, ModelWERRequest
import Levenshtein
...
class Model():
    ...
    async def calculate_wer(self, request: ModelWERRequest):
        error = wer(request.kenlm_transcription, request.asr_transcription)
        error = round(error, 2)
        result = {
            "asr_transcription": request.asr_transcription,
            "kenlm_transcription": request.kenlm_transcription,
            "wer": error
        }
